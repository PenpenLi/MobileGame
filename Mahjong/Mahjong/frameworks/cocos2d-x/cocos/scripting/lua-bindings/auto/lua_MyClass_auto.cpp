...
int lua_MyClass_MyClass_GetFoo(lua_State* tolua_S)
{
    int argc = 0;
    MyClass* cobj = nullptr;
    bool ok  = true;
    ...
    if (!cobj) 
    {
        tolua_error(tolua_S,"invalid 'cobj' in function 'lua_MyClass_MyClass_GetFoo'", nullptr);
        return 0;
    }
#endif
    ...
    if (argc == 0) 
    {
        if(!ok)
        {
            tolua_error(tolua_S,"invalid arguments in function 'lua_MyClass_MyClass_GetFoo'", nullptr);
            return 0;
        }
        int ret = cobj->GetFoo();
    ...
    luaL_error(tolua_S, "%s has wrong number of arguments: %d, was expecting %d \n", "MyClass:GetFoo",argc, 0);
    return 0;

#if COCOS2D_DEBUG >= 1
    tolua_lerror:
    tolua_error(tolua_S,"#ferror in function 'lua_MyClass_MyClass_GetFoo'.",&tolua_err);
#endif

    return 0;
}
int lua_MyClass_MyClass_SetFoo(lua_State* tolua_S)
{
    int argc = 0;
    MyClass* cobj = nullptr;
    bool ok  = true;
    ...
    if (!cobj) 
    {
        tolua_error(tolua_S,"invalid 'cobj' in function 'lua_MyClass_MyClass_SetFoo'", nullptr);
        return 0;
    }
#endif
    ...
    if (argc == 1) 
    {
        ...
        ok &= luaval_to_int32(tolua_S, 2,(int *)&arg0, "MyClass:SetFoo");
        if(!ok)
        {
            tolua_error(tolua_S,"invalid arguments in function 'lua_MyClass_MyClass_SetFoo'", nullptr);
            return 0;
        }
        cobj->SetFoo(arg0);
    ...
    luaL_error(tolua_S, "%s has wrong number of arguments: %d, was expecting %d \n", "MyClass:SetFoo",argc, 1);
    return 0;

#if COCOS2D_DEBUG >= 1
    tolua_lerror:
    tolua_error(tolua_S,"#ferror in function 'lua_MyClass_MyClass_SetFoo'.",&tolua_err);
#endif
...
int lua_register_MyClass_MyClass(lua_State* tolua_S)
{
    ...
    tolua_beginmodule(tolua_S,"MyClass");
        ...
        tolua_function(tolua_S,"GetFoo",lua_MyClass_MyClass_GetFoo);
        tolua_function(tolua_S,"SetFoo",lua_MyClass_MyClass_SetFoo);
